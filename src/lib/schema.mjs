...
const url = (value, name) => { text(value, name); const u = new URL(value); if (!['http:', 'https:'].includes(u.protocol)) throw new Error(`${name} must use HTTP(S)`); };
...
export function validateItem(item) {
  ...
  if (['product', 'ui'].includes(item.category)) { url(item.image?.url, 'image.url'); text(item.image?.licenseStatus, 'image.licenseStatus'); }
  return item;
}
