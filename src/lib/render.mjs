...
function pipelineHtml(run) {
  ...
  const state = escapeHtml(run.state), snapshot = shortVersion(run.input_snapshot_hash);
  return `<section class="band pipeline" id="run-pipeline" data-toc-target><div class="pipeline-head"><div><h2>运行流程 / Execution flow</h2><p class="en">${escapeHtml(run.date)} · ${state} · snapshot <code>${snapshot}</code></p></div><span class="run-schema">${escapeHtml(run.execution_context_schema)}</span></div><ol class="run-stages">${rows}</ol></section>`;
}
...
export function renderHtml(report, { run = null, showPipeline = Boolean(run) } = {}) {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>DesignSignal ${escapeHtml(view.date)}</title><style>${REPORT_CSS}
