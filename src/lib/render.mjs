...
const itemHtml = item => {
  const analysis = [['evidence', '证据 / Evidence'], ['method', '方法 / Method'], ['novelty', '新意 / Novelty'], ['limits', '局限 / Limits'], ['whyLearn', '学习价值 / Why learn'], ['studyAction', '学习行动 / Study action']]
    .map(([field, label]) => `<section class="analysis"><h3>${label}</h3><p>${escapeHtml(item.analysis.zh[field])}</p><p class="en">${escapeHtml(item.analysis.en[field])}</p></section>`).join('');
  ...
  <div class="signal-head">${item.image ? `<img src="${escapeHtml(imageSource(item.image))}" alt="" loading="lazy" referrerpolicy="no-referrer">` : ''}<div><p class="meta">${escapeHtml(item.category.toUpperCase())} · ${escapeHtml(item.id)} · ${(item.confidence * 100).toFixed(0)}%</p><h2>${escapeHtml(item.title.zh)}</h2><p class="title-en">${escapeHtml(item.title.en)}</p>${biHtml(item.synopsis)}<p class="tags">337 · ${item.exam['337'].map(escapeHtml).join(' · ')}</p><p class="tags">902 · ${item.exam['902'].map(escapeHtml).join(' · ')}</p></div></div>
  <div class="analysis-grid">${analysis}</div>
  <section class="provenance"><h3>引用、缓存与权利 / Citations, cache, and rights</h3><div class="provenance-grid"><div><h4>Citations</h4><ul>${citations}</ul></div><div><h4>Cached assets</h4><ul>${assets}</ul></div><div><h4>Rights</h4><p>${escapeHtml(item.rights?.access || 'unknown')} · ${escapeHtml(item.rights?.licenseStatus || 'unknown')}</p><p class="en">${escapeHtml(item.rights?.author || item.image?.author || item.source.name)} · ${escapeHtml(item.rights?.institution || item.image?.institution || item.source.name)}</p></div></div></section>
  </article>`;
};
